...
FLASHMEM void MD_MIDIFile::initialise(void)
{
...
FLASHMEM void MD_MIDIFile::synchTracks(void)
{
...
FLASHMEM void MD_MIDIFile::close()
...
FLASHMEM void MD_MIDIFile::setTempoAdjust(int16_t t)
{
  if ((t + _tempo) > 0) _tempoDelta = t;
  calcTickTime();
}

FLASHMEM void MD_MIDIFile::setTempo(uint16_t t)
{
  if ((_tempoDelta + t) > 0) _tempo = t;
  calcTickTime();
}

FLASHMEM void MD_MIDIFile::setTimeSignature(uint8_t n, uint8_t d)
{
  _timeSignature[0] = n;
  _timeSignature[1] = d;
  calcTickTime();
...
FLASHMEM void MD_MIDIFile::setTicksPerQuarterNote(uint16_t ticks) 
{
  _ticksPerQuarterNote = ticks;
  calcTickTime();
}

FLASHMEM void MD_MIDIFile::setMicrosecondPerQuarterNote(uint32_t m)
...
FLASHMEM void MD_MIDIFile::calcTickTime(void) 
...
FLASHMEM bool MD_MIDIFile::isEOF(void)
{
  bool bEof = true;
...
FLASHMEM void MD_MIDIFile::pause(bool bMode)
...
FLASHMEM void MD_MIDIFile::restart(void)
...
FLASHMEM uint16_t MD_MIDIFile::tickClock(void)
...
FLASHMEM boolean MD_MIDIFile::getNextEvent(void)
{
  uint16_t  ticks;
...
FLASHMEM void MD_MIDIFile::processEvents(uint16_t ticks)
{
  uint8_t n;
...
FLASHMEM int MD_MIDIFile::load(const char *fname) 
...
// Return one of the E_* error codes
{
  ...
  if (dat16 > MIDI_MAX_TRACKS)
  {
    // _fd.close();
    // return(E_TRACKS);
      _trackCount = MIDI_MAX_TRACKS; //set to 16 if there are more that 16 tracks - most times these are empty anyway
  }
  else
  _trackCount = dat16;
