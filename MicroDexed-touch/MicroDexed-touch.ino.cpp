# 1 "C:\\Users\\azoth\\AppData\\Local\\Temp\\tmpwwv5802u"
#include <Arduino.h>
# 1 "C:/Users/azoth/Documents/GitHub/totem/MicroDexed-touch/MicroDexed-touch.ino"
# 25 "C:/Users/azoth/Documents/GitHub/totem/MicroDexed-touch/MicroDexed-touch.ino"
#include <limits.h>
#include "config.h"
#include "scope.h"
#include <Audio.h>
  ...
  {granularPoly, 0, granular_filter_l,0},
   {granularPoly, 0, granular_filter_r,0},
    ...
    {granular_filter_r, 0, master_mixer_r, MASTER_MIX_CH_GRANULAR},
         ...
         {microsynth_noise[0], 0, microsynth_envelope_noise[0], 0},
         {microsynth_noise[1], 0, microsynth_envelope_noise[1], 0},

         {microsynth_envelope_noise[0], 0, microsynth_filter_noise[0], 0},
         {microsynth_envelope_noise[1], 0, microsynth_filter_noise[1], 0},

         {microsynth_filter_noise[0], 0, microsynth_mixer_filter_noise[0], 0},
         ...
         {microsynth_main_mixer_l, 0, sidechain_mixer_l, MASTER_MIX_CH_MICROSYNTH},
            ...
            {microsynth_mixer_filter_osc[0], 0, microsynth_peak_osc_0, 0},
            {microsynth_mixer_filter_osc[1], 0, microsynth_peak_osc_1, 0},
            {microsynth_mixer_filter_noise[0], 0, microsynth_peak_noise_0, 0},
...
FLASHMEM uint32_t crc32(byte* calc_start, uint16_t calc_bytes);
...
#line 605 "C:/Users/azoth/Documents/GitHub/totem/MicroDexed-touch/MicroDexed-touch.ino"
FLASHMEM void create_audio_dexed_chain(uint8_t instance_id)
{
  ...
  if (instance_id == 0 || instance_id == 1)
  {
    ...
    dynamicConnections.emplace_back(new AudioConnection(*global_delay_in_mixer[instance_id], 0, global_delay_filter_mixer[instance_id], 0));

    dynamicConnections.emplace_back(new AudioConnection(global_delay_filter[instance_id], 0, global_delay_filter_mixer[instance_id], 1));
    dynamicConnections.emplace_back(new AudioConnection(global_delay_filter[instance_id], 1, global_delay_filter_mixer[instance_id], 2));
    dynamicConnections.emplace_back(new AudioConnection(global_delay_filter[instance_id], 2, global_delay_filter_mixer[instance_id], 3));
...
FLASHMEM void create_audio_braids_chain(uint8_t instance_id)
{
  ...
  dynamicConnections.emplace_back(new AudioConnection(*braids_envelope[instance_id], 0, *braids_mixer_filter[instance_id], 3));
...
FLASHMEM void create_audio_drum_chain(uint8_t instance_id)
{
  ...
  Drum_filter[instance_id]->setLowpass(0, 20000, 0.2);
  Drum_filter[instance_id + NUM_DRUMS]->setLowpass(0, 20000, 0.2);
#endif
...
# 979 "C:/Users/azoth/Documents/GitHub/totem/MicroDexed-touch/MicroDexed-touch.ino"
...
#if (defined BUTTON_LEDS && defined MCP_23008) || (defined BUTTON_LEDS && defined MCP_23017)
# 1194 "C:/Users/azoth/Documents/GitHub/totem/MicroDexed-touch/MicroDexed-touch.ino"
#endif
  ...
  sgtl5000.volume(SGTL5000_HEADPHONE_VOLUME, SGTL5000_HEADPHONE_VOLUME);
...
#if defined (RGB_ENCODERS) && defined (MCP_23008) || defined (RGB_ENCODERS) && defined (MCP_23017)
  if (digitalRead(BUT_R_PIN) || digitalRead(BUT_L_PIN))
    bootup_performance_loading = false;
#else
  if (digitalRead(BUT_R_PIN) == false || digitalRead(BUT_L_PIN) == false)
    bootup_performance_loading = false;
#endif


#ifdef ONBOARD_BUTTON_INTERFACE
  if (digitalRead(BI_BUTTON_A) == false || digitalRead(BI_BUTTON_B) == false)
    bootup_performance_loading = false;
#endif
...
#if defined(TEENSY_AUDIO_BOARD) && defined(SGTL5000_AUDIO_THRU)
  audio_thru_mixer_r.gain(0, VOL_MAX_FLOAT);
  audio_thru_mixer_l.gain(0, VOL_MAX_FLOAT);
#ifdef TEENSY_AUDIO_BOARD
  audio_thru_mixer_r.gain(1, VOL_MAX_FLOAT);
  audio_thru_mixer_l.gain(1, VOL_MAX_FLOAT);
#else
  ...
  finalized_mixer_l.gain(0, VOL_MAX_FLOAT);
  finalized_mixer_r.gain(0, VOL_MAX_FLOAT);
  finalized_mixer_l.gain(1, 0);
  finalized_mixer_r.gain(1, 0);
  finalized_mixer_l.gain(2, 1.0);
  finalized_mixer_r.gain(2, 1.0);
  ...
  for (int i = 0; i < NUM_STATIC_PITCHED_SAMPLES; i++) {
    char temp_name[36];
  ...
  if (!SD.exists("/DEXED/0/0") || !SD.exists("/DEXED/0/99") || !SD.exists("/PERFORMANCE") || !SD.exists("/DRUMS"))
  {
    ...
    if ((count_omni() != 0 || count_midi_channel_duplicates(false) != 0) && configuration.sys.skip_midi_channel_warning == false)
      LCDML.OTHER_jumpToFunc(UI_func_midi_channels);
    else
      if (seq.clock == 1 && configuration.sys.skip_midi_channel_warning == false)
...
FLASHMEM void sub_step_recording(bool touchinput, uint8_t touchparam)
{
  ...
  {
    if (touchinput && touchparam == 1)
    {
      seq.note_in = 0;
      seq.note_in_velocity = 0;
    }
    else if (touchinput && touchparam == 2 && seq.content_type[seq.active_pattern] != 0)
    {
      seq.note_in = 130;
      seq.note_in_velocity = 0;
    }
...

    {
      ...
      if (cur_step < 16)
      {
        ...
        if (touchparam == 0)
        {
          if (seq.content_type[seq.active_pattern] == 0 && get_sample_note(activeSample) > 209 && ts.virtual_keyboard_instrument != VK_DRUMS)
          {
        ...
        if (seq.auto_advance_step > 0)
        {
          ...
          else
          {
            ...
            else
            {
              ...
              draw_button_on_grid(36, 1, "STEP", "RECORD", 1);
            }
          }
        }
      }
...
#if defined GLOW
float glow_smoothness_pts = 25;
float glow_gamma = 0.25;
float glow_beta = 0.5;
float glow_bright = 0;
int glow_count = 0;
uint16_t glow_x;
...
# 2414 "C:/Users/azoth/Documents/GitHub/totem/MicroDexed-touch/MicroDexed-touch.ino"
...
void loop()
{
  ...
  if (microsynth_control_rate > MICROSYNTH_CONTROL_RATE_MS)
  {
  ...
  if (braids_control_rate > BRAIDS_CONTROL_RATE_MS)
  {
    ...
    if (LCDML.FUNC_getID() == LCDML.OTHER_getIDFromFunction(UI_func_voice_select))
    {
      for (uint8_t instance_id = 0; instance_id < NUM_DEXED; instance_id++)
      {
        ...
        if (midi_decay_timer_dexed > MIDI_DECAY_TIMER && midi_decay_dexed[instance_id] > 0)
        {
          ...
          display.fillRect(167 + (instance_id * 12), 19, 5, 8 - midi_decay_dexed[instance_id], COLOR_BACKGROUND);
          display.fillRect(167 + (instance_id * 12), 27 - midi_decay_dexed[instance_id], 5, midi_decay_dexed[instance_id], COLOR_PITCHSMP);
        }
    ...
    else if (LCDML.FUNC_getID() == LCDML.OTHER_getIDFromFunction(UI_func_microsynth))
    {
      for (uint8_t instance_id = 0; instance_id < NUM_MICROSYNTH; instance_id++)
      {
        ...
        if (midi_decay_timer_microsynth > MIDI_DECAY_TIMER && midi_decay_microsynth[instance_id] > 1)
        {
          ...
          display.fillRect(13 * 6 + (instance_id * 12), 10, 5, 8 - midi_decay_microsynth[instance_id], COLOR_BACKGROUND);
          display.fillRect(13 * 6 + (instance_id * 12), 18 - midi_decay_microsynth[instance_id], 5, midi_decay_microsynth[instance_id], COLOR_PITCHSMP);
        }
...
void Multi_Sample_Player(byte inNumber, byte inVelocity, byte instance)
{
  ...
  if (slot != 99)
  {
    ...
    set_filter(msz[instance][y].psram_entry_number,slot);
        ...
        if (drum_config[y].numChannels == 1)
        {
...
#endif
# 3429 "C:/Users/azoth/Documents/GitHub/totem/MicroDexed-touch/MicroDexed-touch.ino"
      }
    }
  }
}
      ...
      if (seq.recording == false || (seq.recording && inNumber != seq.note_in))
      {
        ...
        for (uint8_t instance_id = 0; instance_id < NUM_MICROSYNTH; instance_id++)
        {
          ...
          if (microsynth[instance_id].midi_channel == inChannel)
          {
            ...
            if (microsynth[instance_id].lfo_mode > 0)
...
set_filter(d,slot);
            ...
            if (drum_config[d].drum_data != NULL && drum_config[d].len > 0 && drum_config[d].len < 10000000) {
              if ((d >= 0 && d < NUM_DRUMSET_CONFIG - 1) || (d >= 210 && d < 216)) {
                ...
                const uint32_t channelMask = ~(drum_config[d].numChannels - 1);
        ...
        if (inChannel == slices_midi_channel)
        {
          ...
          if (slot != 99 && selected_slice_sample[s] != 99)
          {
            ...
            if (drum_config[selected_slice_sample[s]].numChannels == 1)
            {
...
# 4069 "C:/Users/azoth/Documents/GitHub/totem/MicroDexed-touch/MicroDexed-touch.ino"
    drum_counter++;
  ...
  if (device == 0)
  {
    for (uint8_t instance_id = 0; instance_id < NUM_MICROSYNTH; instance_id++)
    {
      ...
      if (inChannel == microsynth[instance_id].midi_channel)
      {
        ...
        if (inNumber == MIDI_C8 || microsynth[instance_id].trigger_noise_with_osc)
...
void handleControlChange(byte inChannel, byte inCtrl, byte inValue)
{
  ...
  else
  {
    ...
    {
# 4286 "C:/Users/azoth/Documents/GitHub/totem/MicroDexed-touch/MicroDexed-touch.ino"
    ...
    if (len == 7)
    {
      ...
      else if ((sysex[3] & 0x7c) >> 2 == 2)
      {
        ...
        case 77:
          ...
          break;
        default:
          MicroDexed[instance_id]->setVoiceDataElement(sysex[4], sysex[5]);
          break;
        }
        MicroDexed[instance_id]->ControllersRefresh();
      }
...
void handleClock(void)
{
  if (seq.clock == 1 && configuration.sys.ext_clock == false)
  {
    sequencer();


    if (midi_bpm_counter % 12 == 0)
    {
      midi_bpm = (round(60000.0f / float(midi_bpm_timer * 2)));
...
FLASHMEM void handleStart(void)
{
  ...
  if (LCDML.FUNC_getID() != LCDML.OTHER_getIDFromFunction(UI_func_information))
  {
    ...
    else if (LCDML.FUNC_getID() == 255 && ts.keyb_in_menu_activated == false)
  ...
  {
    display.fillRect(36 * CHAR_width_small, CHAR_height_small, button_size_x * CHAR_width_small, CHAR_height_small * button_size_y, COLOR_BACKGROUND);
  }
  ...
  {
    liveSeq.onStarted();
    sequencer_timer.begin(sequencer, seq.tempo_ms / (seq.ticks_max + 1), true);
  }
...
bool checkMidiChannelDexed(byte inChannel, uint8_t instance_id)
{
  ...
  {
# 5282 "C:/Users/azoth/Documents/GitHub/totem/MicroDexed-touch/MicroDexed-touch.ino"
    return (false);
  }
  return (true);
}
...
{
# 5329 "C:/Users/azoth/Documents/GitHub/totem/MicroDexed-touch/MicroDexed-touch.ino"
...
FLASHMEM void check_configuration_dexed(uint8_t instance_id)
{
  ...
  configuration.dexed[instance_id].midi_channel = constrain(configuration.dexed[instance_id].midi_channel, MIDI_CHANNEL_MIN, MIDI_CHANNEL_MAX + 1);
...
# 5879 "C:/Users/azoth/Documents/GitHub/totem/MicroDexed-touch/MicroDexed-touch.ino"
  ...
  reverb_mixer_r.gain(REVERB_MIX_CH_DRUMS, 1.0);
  reverb_mixer_l.gain(REVERB_MIX_CH_DRUMS, 1.0);

  reverb_mixer_r.gain(REVERB_MIX_CH_EPIANO, mapfloat(configuration.fx.ep_reverb_send, REVERB_SEND_MIN, REVERB_SEND_MAX, 0.0, 1.0));
  reverb_mixer_l.gain(REVERB_MIX_CH_EPIANO, mapfloat(configuration.fx.ep_reverb_send, REVERB_SEND_MIN, REVERB_SEND_MAX, 0.0, 1.0));
